...
df.insert(4, "Age", column_to_move)
...
cols_drop=['Leather interior', 'Levy', 'Mileage_bin', 'EV_bin', 'Turbo', 'Cylinders']
...
print('final_all_df: ', final_all_df)
...
# save
joblib.dump(model_rf, "my_random_forest_2.joblib")
...
# save
joblib.dump(encoding, 'cats_encoding_2.bin', compress=True)

# load
#loaded_cats_encoding = joblib.load("cats_encoding.bin")

import joblib
import numpy as np

# save
joblib.dump(scaler, 'std_scaler_2.bin', compress=True)
...
# Mileage_bin              5.0
# EV_bin                   4.0
# Manufacturer            32.0 = LEXUS
# Model                 1242.0 = RX 450
# Category                 4.0 = Jeep
# Leather interior         1.0 = Yes
# Fuel type                2.0 = Hybrid
# Gear box type            0.0 = Automatic
# Drive wheels             0.0 = 4x4
# Doors                    1.0 = 4_5
# Wheel                    0.0 = Left wheel
# Color                   12.0 = Silver
...
user_car_filterout_cats = {key: user_car[key] for key in categry_columns}
user_car_filterout_cats

encoded_user_car_filterout_cats = loaded_cats_encoding.transform(np.array([list(user_car_filterout_cats.values())], dtype=object))
encoded_user_car_filterout_cats
...
user_car.update(user_car_filterout_cats)
user_car.keys()

user_car_values_scaled = loaded_scaler.transform(np.array(list(user_car.values())).reshape(1, -1))
user_car_values_scaled

prediction = int(np.exp(loaded_rf.predict(user_car_values_scaled))[0])
print('Price Prediction: ', prediction)

df["Color"].value_counts().to_dict().keys()
